...
2. `inspect` 能读取最新 run 或指定 run，并展示状态、轮次、最终 record 和候选 artifact。
3. `diff` 能对比源文件和候选 workspace。
4. `apply --dry-run` 不写源文件，只验证当前候选能否被应用。
5. `apply --verify` 显式写回源文件，并运行该 run 记录的验证命令。
6. 源文件在 run 开始后发生变化时，`apply` 默认拒绝覆盖；只有人工确认后才使用 `--force`。
...
```bash
gh release download v0.2.0 -R ckken/loopsmith -p 'loopsmith-v0.2.0-aarch64-apple-darwin.tar.gz' -D /tmp/loopsmith-release
tar -xzf /tmp/loopsmith-release/loopsmith-v0.2.0-aarch64-apple-darwin.tar.gz -C /tmp/loopsmith-release
/tmp/loopsmith-release/loopsmith-v0.2.0-aarch64-apple-darwin/loopsmith doctor
```
