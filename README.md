...
<p align="center">
  <em>面向 Codex 的本地 AI 工作流闭环编排 CLI。</em>
</p>
...
`loopsmith` 是一个面向 Codex CLI 的本地 AI 工作流闭环编排 CLI。它把 `codex exec` 包装成可审计、可验证、可复用的本地 loop，让模型输出先进入候选工作区，再通过验证反馈、审计记录和人工确认进入源工作区。

当前实现方向是 **Rust 单二进制 CLI**。仓库已具备可试点的本地工作流闭环：`doctor` 能检测本机 Codex CLI，`run` 能复制当前项目到候选工作区、按 profile 触发一个或多个只读 review agent、调用单 writer/action 阶段生成候选结果、运行机械验证、执行 lifecycle hooks，并写入可审计记录。

## 核心价值

Loopsmith 的重点不是“让 AI 多修几轮”，而是把一次 AI 产出变成可治理的本地工作流：

- **反馈驱动**：每轮验证输出都会成为下一轮输入，而不是让模型凭感觉继续改。
- **候选隔离**：模型只在 `runs/<run-id>/` 下的候选 workspace 工作，默认不直接覆盖源文件。
- **可验证**：成功与否优先由本地验证命令决定，例如 test、lint、schema check、文档构建或自定义脚本。
- **可审计**：prompt、schema、answer、stdout/stderr、validation、remaining delta、hooks 都会落盘。
- **可回放**：`inspect` / `diff` / `apply --dry-run` / `apply --verify` 让人工能复盘并显式采纳候选结果。

这个模式适用于代码修复、测试补齐、文档更新、配置迁移、schema 调整、prompt 改写和静态站点更新等任务。前提是目标 artifact 边界清晰，并且有可信反馈可以衡量结果。

## 模式来源

Loopsmith 参考 OpenAI Cookbook 的 [Build iterative repair loops with Codex](https://developers.openai.com/cookbook/examples/codex/build_iterative_repair_loops_with_codex)。那篇文章虽然以 documentation repair 为示例，但核心模式是 closed-loop agent workflow：agent 生成输出、验证输出、再使用反馈改进下一轮。文中的 notebook 只是示例；同一模式适用于任何能被可信反馈衡量的 artifact。

## 目标

第一版 CLI 聚焦一个最小闭环：

1. 读取目标 artifact 和本轮目标。
2. 调用 `codex exec` 进行结构化 review。
3. 在候选工作区中执行 bounded action，生成下一版候选结果。
4. 执行机械验证命令，例如 `cargo test`、`npm test`、`pytest`、文档构建或 schema 校验。
...
- Codex 只负责 review 和候选结果生成，不负责最终判定。
...
- `loopsmith run`：创建候选 workspace，执行 review / act / verify / record 循环。
...
- `artifact`：目标 artifact 文件路径。
- `goal`：本轮工作流目标。
- `verify`：机械验证命令。
- `max_iterations`：最大迭代次数。
- `model`：默认传给 `codex exec --model` 的模型。
- `review_model`：可选，review 阶段模型；未配置时回退到 `model`。
- `repair_model`：可选，writer/action 阶段模型；未配置时回退到 `model`。字段名沿用 OpenAI 示例中的 repair phase，实际可用于任何候选结果生成任务。
...
多 review agent 示例见 [examples/multi-review-loop.json](examples/multi-review-loop.json)。`multi-review` 会触发 `correctness`、`tests`、`docs` 三个只读 reviewer，并把 findings 合并后交给单 writer/action 阶段。

当前建议模型：

- `gpt-5.5`：复杂任务、跨模块分析和高风险变更的默认稳妥选择。
- `gpt-5.4`：日常代码、文档和配置 review，适合放在 `review_model`。
- `gpt-5.4-mini`：更快、更省的候选结果生成，适合低风险 `repair_model`。
- `gpt-5.3-codex-spark`：超快编码模型；如果当前账号/环境可用，可用于低风险 action 或快速试跑。
...
- `config`：读取和校验配置。
- `schema`：生成 review/action JSON schema。
...
- Rust CLI 骨架已实现。
- 已用当前项目真实跑通一轮 `codex exec` review/action/verify loop。
- 已具备可试点 vibecoding 工作流：`run` 生成候选结果，`inspect` 查看状态，`diff` 对比候选，`apply --dry-run` 做写回前检查，`apply --verify` 显式写回并重新验证。
- 已支持 workflow profile、多个只读 review agent、单 writer/action 阶段，以及 `pre_run` / `post_iteration` / `pre_apply` / `post_apply` / `on_failure` hooks。
